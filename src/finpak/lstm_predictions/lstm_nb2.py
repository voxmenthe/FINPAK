...
def calculate_features(prices):
    ...
    if len(prices) > 1:
        ...
        pct_change_1d = np.insert(pct_change_1d, 0, 0)
    else:
        pct_change_1d = np.array([0])
    
    # Percentage difference from 30-day moving average
    if len(prices) >= 30:
        ma_30 = pd.Series(prices).rolling(window=30).mean().iloc[-1]
        pct_diff_ma30 = (prices[-1] - ma_30) / ma_30
    else:
        pct_diff_ma30 = 0
    
    # 5-day percentage change
    if len(prices) >= 5:
        pct_change_5d = (prices[-1] - prices[-5]) / prices[-5]
    else:
        pct_change_5d = 0
    
    # 15-day percentage change
    if len(prices) >= 15:
        pct_change_15d = (prices[-1] - prices[-15]) / prices[-15]
    else:
        pct_change_15d = 0
    
    return np.array([pct_change_1d[-1], pct_diff_ma30, pct_change_5d, pct_change_15d])
...
def make_prediction(start_idx):
    ...
    with torch.no_grad():
        for _ in range(prediction_length):
            ...
            last_price = prices[start_idx + sequence_length - 1]
            new_price = last_price * (1 + pred.item())
            new_features = calculate_features(np.array([new_price]))
