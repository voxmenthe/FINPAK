...
class StockDataset(Dataset):
    def __init__(
        ...
        """
        if len(continuous_features) != len(continuous_targets) or len(categorical_features) != len(continuous_targets):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, Tuple[torch.Tensor, Optional[torch.Tensor]]]:
        ...
        categorical_target = None if self.categorical_targets is None else self.categorical_targets[idx + self.sequence_length - 1]
