...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if past_k is not None and past_v is not None:
            # Concatenate past k and v with current k and v
            k = torch.cat([past_k, k], dim=2)  # (B, n_heads, T_total, head_dim)
            v = torch.cat([past_v, v], dim=2)
            past_length = past_k.size(2)
        else:
            past_length = 0

        T_total = k.size(2)  # Total length including past

        # Scaled dot-product attention with causal mask
        att = (q @ k.transpose(-2, -1)) / (self.head_dim ** 0.5)  # (B, n_heads, T, T_total)

        # Add positional bias to attention scores if using positional encoding
        pos_bias = self.pos_enc(T_total)
        # Reshape to match attention heads
        pos_bias = pos_bias.view(T_total, T_total, self.n_heads, self.head_dim)
        # Sum over head_dim to get (T_total, T_total, n_heads)
        pos_bias = pos_bias.sum(dim=-1)
        pos_bias = pos_bias.permute(2, 0, 1)  # (n_heads, T_total, T_total)
        pos_bias = pos_bias.unsqueeze(0)  # (1, n_heads, T_total, T_total)

        # Slice position bias to match current query length T
        pos_bias = pos_bias[:, :, past_length:, :]  # (1, n_heads, T, T_total)
        att = att + pos_bias

        # Apply causal mask
        causal_mask = torch.triu(
            torch.ones(T, T_total, device=att.device, dtype=torch.bool),
        ...
        # Return output and new k, v for caching
        return out, k, v
...
class MultiScaleAttention(nn.Module):
    ...
    def forward(
        self, 
        x: torch.Tensor, 
        past_kvs: Optional[List[Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]]] = None
    ) -> Tuple[torch.Tensor, List[Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]]]:
        B, T, C = x.shape
        
        # Initialize past_kvs if not provided
        if past_kvs is None:
            past_kvs = [None] * len(self.scales)
            
        # Process each temporal scale
        scale_outputs = []
        new_past_kvs = []
        for idx, (scale, attention) in enumerate(zip(self.scales, self.attention_modules)):
            past_k, past_v = past_kvs[idx] if past_kvs[idx] is not None else (None, None)
            ...
            if scale > 1:
                ...
                # Apply attention without caching for downsampled sequences
                attended, _, _ = attention(x_scaled)
                new_past_kvs.append((None, None))
                ...
                # Apply attention with caching for scale=1
                attended, k, v = attention(x, past_k, past_v)
                new_past_kvs.append((k, v))
                
            scale_outputs.append(attended)
            
        # Combine multi-scale features
        multi_scale = torch.cat(scale_outputs, dim=-1)
        return self.scale_combine(multi_scale), new_past_kvs
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        attn_output, new_past_kvs = self.attention(attn_input, past_kvs)
...
class TimeSeriesDecoder(nn.Module):
    ...
    def forward(
        self, 
        x: torch.Tensor, 
        past_kvs_list: Optional[List[List[Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]]]] = None
    ) -> Tuple[torch.Tensor, List[List[Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]]]]:
        ...
        # Initialize past_kvs_list if not provided
        if past_kvs_list is None:
            past_kvs_list = [None] * len(self.blocks)
            
        new_past_kvs_list = []
        # Pass through decoder blocks
        for block, past_kvs in zip(self.blocks, past_kvs_list):
            x, new_past_kvs = block(x, past_kvs)
            new_past_kvs_list.append(new_past_kvs)
                
        # Use last sequence element for prediction
        x = self.ln_f(x[:, -1])
        return self.output_projection(x), new_past_kvs_list
