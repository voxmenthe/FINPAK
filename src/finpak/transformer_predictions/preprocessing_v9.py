...
from stock_dataset_v9 import StockFeatures
...
def create_stock_features(
    ...
    # Track categorical feature coverage for debug
    categorical_feature_coverage: Dict[str, float] = {}
    ...
    # --- Extra continuous features (v9) ---
    for feature_cfg in config.get('continuous_features_extra', []):
        feature_name = feature_cfg.get('name')
        if feature_name == 'dist_from_Nd_high_over_Kd_range':
            n_days = int(feature_cfg['n'])
            k_days = int(feature_cfg['k'])
            eps = float(feature_cfg.get('eps', 1e-8))
            # rolling N-day high (close)
            n_high = _rolling_max(prices, n_days)
            # rolling K-day high/low (close)
            k_high = _rolling_max(prices, k_days)
            k_low = _rolling_min(prices, k_days)
            denom = torch.clamp(k_high - k_low, min=eps)
            dist = (n_high - prices) / denom
            continuous_features.append(dist)
            continuous_feature_names.append(f'dist_from_{n_days}d_high_over_{k_days}d_range')
        else:
            raise ValueError(f"Unknown continuous_features_extra name: {feature_name}")

    # --- Categorical features (v9) ---
    for feature_cfg in config.get('categorical_features', []):
        feature_name = feature_cfg.get('name')
        feature_type = feature_cfg.get('type')
        if feature_type == 'nd_high_low_asym':
            n_days = int(feature_cfg['n'])
            k_days = int(feature_cfg['k'])
            n_high = _rolling_max(prices, n_days)
            n_low = _rolling_min(prices, n_days)
            hit_high = _recent_event_mask(prices, n_high, k_days, mode='high')
            hit_low = _recent_event_mask(prices, n_low, k_days, mode='low')
            feature_a = (hit_high & (~hit_low)).long()
            feature_b = (hit_low & (~hit_high)).long()
            categorical_features.append(feature_a)
            categorical_feature_names.append(f'hit_{n_days}d_high_in_last_{k_days}d_and_not_low')
            categorical_features.append(feature_b)
            categorical_feature_names.append(f'hit_{n_days}d_low_in_last_{k_days}d_and_not_high')
            if debug:
                categorical_feature_coverage[categorical_feature_names[-2]] = feature_a.float().mean().item()
                categorical_feature_coverage[categorical_feature_names[-1]] = feature_b.float().mean().item()
        elif feature_type == 'roc_gt_shift':
            p = int(feature_cfg['p'])
            nperiod = int(feature_cfg['nperiod'])
            k_days = int(feature_cfg['k'])
            roc_p = calculate_returns(prices, p)
            roc_n = calculate_returns(prices, nperiod)
            roc_n_shift = _shift_series(roc_n, k_days)
            feature_c = (roc_p > roc_n_shift).long()
            categorical_features.append(feature_c)
            categorical_feature_names.append(f'roc_{p}d_gt_roc_{nperiod}d_{k_days}d_ago')
            if debug:
                categorical_feature_coverage[categorical_feature_names[-1]] = feature_c.float().mean().item()
        elif feature_type == 'roc_lt_shift':
            p = int(feature_cfg['p'])
            nperiod = int(feature_cfg['nperiod'])
            k_days = int(feature_cfg['k'])
            roc_p = calculate_returns(prices, p)
            roc_n = calculate_returns(prices, nperiod)
            roc_n_shift = _shift_series(roc_n, k_days)
            feature_d = (roc_p < roc_n_shift).long()
            categorical_features.append(feature_d)
            categorical_feature_names.append(f'roc_{p}d_lt_roc_{nperiod}d_{k_days}d_ago')
            if debug:
                categorical_feature_coverage[categorical_feature_names[-1]] = feature_d.float().mean().item()
        else:
            raise ValueError(f"Unknown categorical feature type: {feature_type}")
    
    # Stack features
    n_samples = prices.shape[0]
    if continuous_features:
        continuous_tensor = torch.stack(continuous_features, dim=1)
    else:
        continuous_tensor = torch.empty((n_samples, 0), dtype=prices.dtype, device=prices.device)

    if categorical_features:
        categorical_tensor = torch.stack(categorical_features, dim=1).long()
    else:
    ...
    if debug:
        ...
        if categorical_feature_coverage:
            print("\nCategorical feature coverage (mean of flags):")
            for name, mean_val in categorical_feature_coverage.items():
    ...
    categorical_targets = torch.stack(categorical_target_list, dim=1).long() if categorical_target_list else None
    ...
    # Include categorical/extra continuous feature lookbacks
    cat_lookbacks = _categorical_lookback_days(config.get('categorical_features', []))
    extra_cont_lookbacks = _continuous_extra_lookback_days(config.get('continuous_features_extra', []))
    lookback_periods = lookback_periods + cat_lookbacks + extra_cont_lookbacks
    valid_start_idx = max(
...
def _rolling_max(series: torch.Tensor, window: int) -> torch.Tensor:
    max_series = torch.zeros_like(series)
    for i in range(window, len(series) + 1):
        max_series[i - 1] = series[i - window:i].max()
    return max_series


def _rolling_min(series: torch.Tensor, window: int) -> torch.Tensor:
    min_series = torch.zeros_like(series)
    for i in range(window, len(series) + 1):
        min_series[i - 1] = series[i - window:i].min()
    return min_series


def _recent_event_mask(prices: torch.Tensor, rolling_extreme: torch.Tensor, k_days: int, mode: str) -> torch.Tensor:
    if mode not in ('high', 'low'):
        raise ValueError(f"Unknown mode: {mode}")
    # event at time t if price equals rolling extreme at t
    event = prices == rolling_extreme
    # recent event if any event within last k_days (inclusive of t)
    recent = torch.zeros_like(event, dtype=torch.bool)
    for i in range(len(event)):
        start = max(0, i - k_days + 1)
        recent[i] = event[start:i + 1].any()
    return recent


def _shift_series(series: torch.Tensor, k_days: int) -> torch.Tensor:
    if k_days <= 0:
        return series
    padding = torch.zeros(k_days, dtype=series.dtype, device=series.device)
    return torch.cat([padding, series[:-k_days]], dim=0)


def _categorical_lookback_days(features: List[Dict[str, Any]]) -> List[int]:
    lookbacks: List[int] = []
    for cfg in features:
        ftype = cfg.get('type')
        if ftype == 'nd_high_low_asym':
            n_days = int(cfg['n'])
            k_days = int(cfg['k'])
            lookbacks.append(max(n_days, k_days))
        elif ftype in ('roc_gt_shift', 'roc_lt_shift'):
            p = int(cfg['p'])
            nperiod = int(cfg['nperiod'])
            k_days = int(cfg['k'])
            lookbacks.append(max(p, nperiod + k_days))
        else:
            raise ValueError(f"Unknown categorical feature type: {ftype}")
    return lookbacks


def _continuous_extra_lookback_days(features: List[Dict[str, Any]]) -> List[int]:
    lookbacks: List[int] = []
    for cfg in features:
        name = cfg.get('name')
        if name == 'dist_from_Nd_high_over_Kd_range':
            n_days = int(cfg['n'])
            k_days = int(cfg['k'])
            lookbacks.append(max(n_days, k_days))
        else:
            raise ValueError(f"Unknown continuous_features_extra name: {name}")
    return lookbacks
