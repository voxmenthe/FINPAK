...
class TimeSeriesDecoder(nn.Module):
    def __init__(
        ...
        # Remove fixed-size positional embedding and make it dynamic
        self.register_buffer('pos_embedding', None)  # We'll create this in forward pass
    ...
    def forward(self, x: torch.Tensor):
        ...
        # Create or reuse positional embeddings of correct size
        if self.pos_embedding is None or self.pos_embedding.size(1) != T:
            self.register_buffer(
                'pos_embedding',
                nn.Parameter(torch.zeros(1, T, self.blocks[0].attention.d_model, device=x.device)),
                persistent=False
            )
            # Initialize with sinusoidal encoding
            position = torch.arange(T, dtype=torch.float, device=x.device).unsqueeze(1)
            div_term = torch.exp(torch.arange(0, self.blocks[0].attention.d_model, 2, device=x.device) * 
                               (-math.log(10000.0) / self.blocks[0].attention.d_model))
