...
import numpy as np
from data_loading_v9 import create_subset_dataloaders
from timeseries_decoder_v9 import TimeSeriesDecoder
from train_v9 import train_model
import pandas as pd
from finpak.data.fetchers.yahoo import download_multiple_tickers
from ticker_cycler import TickerCycler
from configs_v9 import all_configs
from ticker_configs import train_tickers_v9, val_tickers_v9
...
if __name__ == "__main__":
    ...
    train_tickers = train_tickers_v9
    val_tickers = val_tickers_v9

    if CONFIG['data_params'].get('reverse_tickers', False):
        train_tickers = train_tickers[::-1]
        val_tickers = val_tickers[::-1]

    train_df_fname = 'TRAIN_VAL_DATA/train_df_v9.csv'
    val_df_fname = 'TRAIN_VAL_DATA/val_df_v9.csv'
    ...
    # Continuous extra features
    n_continuous += len(CONFIG['data_params'].get('continuous_features_extra', []))
    ...
    n_categorical += len(CONFIG['data_params'].get('categorical_features', []))
    ...
    # Align model IO sizing with registry when available
    if registry:
        n_continuous = len(registry.get('continuous_features') or [])
        n_categorical = len(registry.get('categorical_features') or [])
