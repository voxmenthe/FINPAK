...
def train_model(
    ...
    best_models = []  # list of (val_loss, epoch, cycle) tuples
            ...
            if train_cycler and train_cycler.has_more_subsets():
                ...
                if valid_checkpoints:
                    ...
                    manage_checkpoints(
                        ...
                        max_global_checkpoints=train_params.get('max_global_checkpoints', 10),
                    )
...
def manage_checkpoints(checkpoint_dir: str, current_subset_checkpoints: List[Tuple[float, int, str, int]], 
                      ...
                      current_cycle: int = 0, max_global_checkpoints: int = 10) -> None:
    ...
    if qualified_checkpoints:
        ...
        best_models.append((best_loss, best_epoch, (current_cycle, best_epoch)))
        ...
        while len(best_models) > max_global_checkpoints:
            # Remove the worst (highest loss) model, never the global best
            worst_idx, worst_entry = max(enumerate(best_models), key=lambda item: item[1][0])
            worst_loss, worst_epoch, (worst_cycle, _) = worst_entry
